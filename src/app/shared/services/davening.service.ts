...
export class DaveningService { // A general service to hold 'global' data relevant all around
    ...
    setErrorMessage(message: string, preventClear = false) {
        ...
        this.preventClearMessagesSubject.next(preventClear); // Set flag
      }
    
      clearErrorMessage() {
        this.errorMessageSubject.next(null);
        this.preventClearMessagesSubject.next(false); // Reset flag
      }
    
    get errorMessage(): string | null {
        return this.errorMessageSubject.value; // Getter to access the latest value
    }

    private successMessageSubject = new BehaviorSubject<string | null>(null);
    successMessage$ = this.successMessageSubject.asObservable();

    private preventClearMessagesSubject = new BehaviorSubject<boolean>(false); // Store as observable
    preventClearMessages$ = this.preventClearMessagesSubject.asObservable();
  
    setSuccessMessage(message: string, preventClear = false) {
      this.successMessageSubject.next(message);
      this.preventClearMessagesSubject.next(preventClear); // Store in BehaviorSubject
    }
  
    clearSuccessMessage() {
      this.successMessageSubject.next(null);
      this.preventClearMessagesSubject.next(false); // Reset properly
    }
  
    shouldClearMessages(): boolean {
      return !this.preventClearMessagesSubject.value; // Retrieve value correctly
    }

    get successMessage(): string | null {
        return this.successMessageSubject.value; // Getter to access the latest value
    }
